...
export function AdminResearchLibrary({onOpen}:{onOpen:()=>void}){
 ...
 async function save(){
  ...
  if(!window.confirm('編集内容を論文に保存しますか？公開中の論文は公開画面にも反映されます。'))return;
  ...
  try{
   ...
   const schoolName=draft.schoolName.trim()||'学校名未登録';
   const registeredName=officialName(fresh.data.data.schoolId);
   const data={...fresh.data.data,title:draft.title.trim(),field:draft.field.trim(),schoolName,schoolNameOverride:registeredName===schoolName?null:schoolName,author:draft.author.trim()||'著者名非公開',...(draft.field.trim()!==fresh.data.data.field?{classificationMode:'manual'}:{})};
   const result=await supabase.rpc('lti_save_records',{ops:[{action:'update',kind:'papers',id:row.id,version,data}]});
   if(result.error)throw result.error;
   setEditing(false);setMessage('変更を保存しました。公開中の論文は公開画面にも反映されます。');
   await cloud.reload();
 ...
 <div className="rounded-xl bg-indigo-50 p-4 text-sm text-indigo-900">LTI運営用：全学校の論文を公開状況にかかわらず閲覧できます。論文情報やAI参考情報はここから編集できます。公開中の論文は保存時に公開画面へ反映され、手動編集ではAIを再実行しません。</div>
 ...
 <ResearchDetail paper={paper} onBack={()=>{if(busy||insight)return;if(editing&&!window.confirm('未保存の編集を破棄して一覧に戻りますか？'))return;setSelected(null);setEditing(false);setMessage('');}}/>
