...
export function ResearchEvaluation({value,onChange,disabled=false}:{value?:Evaluation;onChange?:(v:Evaluation)=>void;disabled?:boolean}){
 ...
 {onChange&&(['reason','evidence','nextStep'] as const).map(k=><div key={k}><label className="text-xs font-bold text-slate-500">{k==='reason'?'評価理由・良い点':k==='evidence'?'本文の根拠（ページ・節など）':'さらに確かめたい点'}{<textarea disabled={disabled} value={v[k]} onChange={e=>update(i,{[k]:e.target.value})} className="block mt-1 w-full border rounded-lg p-2 text-sm text-slate-800" rows={2}/>}</label></div>)}
 </div>)}</section>;
}
