...
import { mergePapers, type Paper } from './supabase/functions/paper-search/search';
export function ScholarlySearch({savedKeys,onSave}:{savedKeys:string[];onSave:(p:Paper)=>void}){
 ...
 const [page,setPage]=useState(-1);const [hasMore,setHasMore]=useState(false);const [onlyReadable,setOnlyReadable]=useState(false);const [order,setOrder]=useState('relevance');
 ...
 return <section className="space-y-4">
  ...
  {displayed.map(p=><article key={p.doi||p.catalogId} className="bg-white border rounded-2xl p-5 space-y-3"><h3 className="font-bold text-gray-900">{p.title}</h3><p className="text-xs text-gray-600">{p.authors||'著者情報なし'} / {p.year||'年不明'} / {p.journal}</p><p className="text-xs text-gray-500">{p.providers.join('・')}{p.paperType==='thesis'||p.paperType==='dissertation'?' / 学位論文（学位種別は掲載先参照）':''}</p><div className="flex flex-wrap gap-4 text-sm"><a href={p.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-emerald-700 underline">掲載情報 ↗</a>{p.documentUrl?<a href={p.documentUrl} target="_blank" rel="noopener noreferrer" className="bg-emerald-50 px-3 py-1 rounded-lg text-emerald-800 font-bold">本文を読む ↗</a>:<span className="text-gray-500">無料本文リンク未取得</span>}<button type="button" onClick={()=>onSave(p)} disabled={savedKeys.includes(p.doi||p.catalogId!)} className="text-emerald-700 font-bold disabled:text-gray-400">{savedKeys.includes(p.doi||p.catalogId!)?'保存済み':'参考文献に保存'}</button></div></article>)}
  ...
  {papers.length>0&&<p className="text-xs text-gray-500">本文リンクはCOREの公開情報に基づきます。リンク切れや掲載先のアクセス制限がある場合があります。並び替え・本文リンクの絞り込みは取得済みの結果に適用されます。</p>}
 </section>;
}
