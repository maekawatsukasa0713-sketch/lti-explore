...
import { theses as titleCatalog, type Thesis } from './thesis-catalog';
import { fullTheses } from './full-theses';
const theses = [...fullTheses, ...titleCatalog];
export const kindLabel = { bachelor: '卒業論文', master: '修士論文', doctoral: '博士論文' };
const accessLabel = { title: '題目のみ', summary: '概要PDF', full: '本文PDF' };
export function ThesisSearch({ savedKeys, onSave }: { savedKeys: string[]; onSave: (p: Thesis) => void }) {
  ...
  return <section className="space-y-4">
    <div className="bg-emerald-50 rounded-xl p-4 text-sm space-y-2"><h3 className="font-bold">大学の卒論・修論を探す</h3><p>標準では本文PDFを確認できた論文 {fullTheses.length}件だけを検索します。概要・題目のみの資料は「公開範囲」を変更すると表示されます。</p><p>収録元：素粒子論研究、愛媛大学、早稲田大学の研究室。全国網羅ではなく、分野に偏りがあります。確認日：2026年9月10日。PDFは公開元で読みます。</p></div>
    ...
    <div className="flex flex-wrap gap-3">
      ...
      <label className="text-sm">大学・掲載元<select value={university} onChange={e=>{setUniversity(e.target.value);setLimit(20);}} className="block border rounded-lg p-2"><option value="">すべての掲載元</option>{Array.from(new Set(theses.map(p=>p.university))).map(v=><option key={v}>{v}</option>)}</select></label>
    ...
    <p className="text-xs text-gray-500">検索はタイトル・著者・分野・年を対象とします。PDF本文の全文検索ではありません。</p>
    ...
    {results.slice(0,limit).map(p=><article key={p.key} className="bg-white border rounded-2xl p-5 space-y-3">
      ...
      <h3 className="font-bold">{p.title}</h3><p className="text-sm text-gray-600">{p.authors} / {p.university} / {p.year}年 / {p.field}</p>
      <div className="flex flex-wrap gap-4 text-sm text-emerald-700"><a href={p.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline">掲載情報 ↗</a>{p.documentUrl&&<a href={p.documentUrl} target="_blank" rel="noopener noreferrer" className="underline">{accessLabel[p.access]}を開く ↗</a>}<button type="button" disabled={savedKeys.includes(p.key)} onClick={()=>onSave(p)} className="font-bold disabled:text-gray-400">{savedKeys.includes(p.key)?'保存済み':'参考文献に保存'}</button></div>
    </article>)}
