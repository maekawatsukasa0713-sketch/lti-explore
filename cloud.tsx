...
export function CloudGate({children}:{children:(p:Profile,logout:()=>Promise<void>)=>React.ReactNode}) {
 ...
 const [email,setEmail]=useState('');const [password,setPassword]=useState('');const [message,setMessage]=useState('');const [mode,setMode]=useState<'login'|'reset'>('login');const [recovery,setRecovery]=useState(recoveryOnArrival);const [passwordAgain,setPasswordAgain]=useState('');
 ...
 if(recovery&&session&&!aal)return waitingScreen('パスワード変更に必要な認証を確認中…');
 if(recovery&&session&&nextAal==='aal2'&&aal!=='aal2')return <div className="min-h-screen p-8"><p className="max-w-lg mx-auto mb-4">パスワードを変更する前に、登録済みの認証アプリで本人確認してください。</p><MfaSetup onDone={()=>{setAuthError('');void supabase.auth.mfa.getAuthenticatorAssuranceLevel().then(({data,error})=>{if(error)setAuthError(error.message);else if(data){setAal(data.currentLevel);setNextAal(data.nextLevel);}});}} onCancel={()=>void signOut()}/>{authError&&<p role="alert">{authError}</p>}</div>;
 if(recovery)return <div className="max-w-md mx-auto p-8 space-y-4"><h1 className="text-xl font-bold">新しいパスワードを設定</h1>{!session?<><p role="alert">再設定リンクが期限切れ、または無効です。再設定メールを送り直してください。</p><button onClick={()=>{window.history.replaceState(null,'',window.location.pathname);setRecovery(false);setEntry(true);setLoginType('email');setMode('reset');}}>再設定メールを送り直す</button></>:<form className="space-y-4" onSubmit={async e=>{e.preventDefault();if(busy)return;setAuthError('');if(password!==passwordAgain){setAuthError('パスワードが一致しません。');return;}setBusy(true);try{const result=await supabase.auth.updateUser({password});if(result.error)throw result.error;setPassword('');setPasswordAgain('');const signedOut=await supabase.auth.signOut({scope:'local'});if(signedOut.error)throw new Error('パスワードは変更済みですが、ログアウトに失敗しました。再読み込みしてください。');window.history.replaceState(null,'',window.location.pathname);setRecovery(false);setSession(null);setProfile(null);setEntry(true);setLoginType('email');setMode('login');setMessage('パスワードを変更しました。新しいパスワードでログインしてください。');}catch(e){setAuthError((e as Error).message||'変更できませんでした。');}finally{setBusy(false);}}}><label className="block">新しいパスワード（12文字以上）<PasswordInput autoComplete="new-password" minLength={12} required value={password} onChange={e=>setPassword(e.target.value)} className="border p-3 w-full"/></label><label className="block">確認用パスワード<PasswordInput autoComplete="new-password" minLength={12} required value={passwordAgain} onChange={e=>setPasswordAgain(e.target.value)} className="border p-3 w-full"/></label><button disabled={busy} className="p-3 rounded-xl bg-emerald-700 text-white">{busy?'変更中…':'パスワードを変更する'}</button></form>}<p role="alert">{authError}</p></div>;
