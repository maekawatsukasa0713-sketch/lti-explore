...
export function CloudGate({children}:{children:(p:Profile,logout:()=>Promise<void>)=>React.ReactNode}) {
 ...
 const [email,setEmail]=useState('');const [password,setPassword]=useState('');const [message,setMessage]=useState('');const [mode,setMode]=useState<'login'|'reset'>('login');const [recovery,setRecovery]=useState(false);
 ...
 const auth=async(e:React.FormEvent)=>{e.preventDefault();setAuthError('');setMessage('');setBusy(true);try{
  if(mode==='reset'){const {error}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:APP_URL});if(error)throw error;setMessage('登録済みの場合、再設定メールが届きます。');}
