...
const registrationGuidance="一括登録の解析では、タイトル・要約・継続研究案・分野分類に加え、LTI運営内部で確認する5観点評価も出力します。evaluationは固定20項目で返します。1=着眼点・独創性、2=再現性、3=手法の妥当性、4=論理の一貫性、5=発展性。1の着眼点・独創性は、高校生ならではの柔軟な発想を重視する。身近な違和感や日常の観察から問いを立てている、既知のテーマを別の角度から見ている、意外な対象や概念を組み合わせている、素朴だが本質的な疑問を研究可能な問いへ育てている場合を高く評価する。テーマの難しさ、専門用語の多さ、設備の豪華さ、大学レベルの高度さは加点要素にしない。奇抜さだけでは加点せず、本文から本人なりの問いや切り口として確認できることを重視する。外部文献を網羅確認していないため『世界初』『前例がない』とは断定しない。各観点についてscore1〜score5、reason1〜reason5、evidence1〜evidence5、nextStep1〜nextStep5を必ず埋めます。scoreは0〜4で、0は本文情報不足による判断保留、1〜4は既存の4段階評価です。reason・evidence・nextStepは本文根拠に基づいて簡潔に書いてください。corrections、advice、nextExperimentsは出力しません。要約は4〜6項目、各500字以内。継続研究案は原則3件、各350〜550字を目安にします。根拠のある提案を優先し、件数を合わせるために内容を水増ししません。文量より正確さを優先し、本文に根拠がない説明を補いません。";
...
Deno.serve(async(req:Request)=>{
 ...
 let registration:Awaited<ReturnType<typeof claimPaper>>|undefined;let stage='start';let quotaUsageId:number|null=null;let refundQuota=async()=>{};
 try{
  ...
  const quota=await fetch(base+'/rest/v1/rpc/lti_claim_ai_quota',{method:'POST',headers:{...authHeaders,'content-type':'application/json'},body:JSON.stringify({request_mode:body.mode}),signal:AbortSignal.timeout(8000)});
