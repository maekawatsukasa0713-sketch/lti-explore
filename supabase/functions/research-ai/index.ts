...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(payload.stop_reason==='max_tokens'||!validResult(result)||(body.mode==='register'&&(!fields.includes(result.classification?.field)||typeof result.classification?.reason!=='string'||result.classification.reason.length>500)))return reply({error:'AIの回答が不完全です。再試行してください。'},502);
