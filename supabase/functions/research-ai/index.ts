...
const registrationEvaluationSchema={type:'object',properties:{focus:evaluationItem,reproducibility:evaluationItem,method:evaluationItem,logic:evaluationItem,development:evaluationItem},required:['focus','reproducibility','method','logic','development'],additionalProperties:false};
export const registrationSchema={type:'object',properties:{title:{type:'string'},summary:{type:'array',minItems:1,items:{type:'string'}},suggestions:{type:'array',minItems:1,items:{type:'object',properties:{title:{type:'string'},description:{type:'string'},tags:{type:'array',items:{type:'string'}}},required:['title','description'],additionalProperties:false}},classification:{type:'object',properties:{field:{type:'string',enum:fields},reason:{type:'string'}},required:['field','reason'],additionalProperties:false},evaluation:registrationEvaluationSchema},required:['title','summary','suggestions','classification','evaluation'],additionalProperties:false};
...
const registrationGuidance="一括登録の解析では、タイトル・要約・継続研究案・分野分類に加え、LTI運営内部で確認する5観点評価も出力します。evaluationはfocus=問い・着眼点、reproducibility=再現性、method=手法の妥当性、logic=論理の一貫性、development=発展性の固定5項目です。各scoreは1〜4または判断不能ならnullとし、reason・evidence・nextStepを本文根拠に基づいて簡潔に書いてください。corrections、advice、nextExperimentsは出力しません。要約は4〜6項目、各500字以内。継続研究案は原則3件、各350〜550字を目安にします。根拠のある提案を優先し、件数を合わせるために内容を水増ししません。文量より正確さを優先し、本文に根拠がない説明を補いません。";
...
export function normalizeRegistrationResult(v:any){
 ...
 const evaluationKeys=['focus','reproducibility','method','logic','development'];
 if(!v.evaluation||typeof v.evaluation!=='object'||Array.isArray(v.evaluation))return reject('evaluation_type');
 const evaluationItems=[];
 for(const key of evaluationKeys){const item=v.evaluation[key];if(!item||typeof item!=='object'||Array.isArray(item))return reject(`evaluation_${key}`);if(![null,1,2,3,4].includes(item.score))return reject(`evaluation_score_${key}`);if(!['reason','evidence','nextStep'].every(k=>typeof item[k]==='string'&&item[k].length<=2000))return reject(`evaluation_text_${key}`);evaluationItems.push({score:item.score,reason:item.reason.trim(),evidence:item.evidence.trim(),nextStep:item.nextStep.trim()});}
  ...
  if(body.mode==='register'){
   const background=async()=>{
    ...
    try{
       ...
       const response=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'},body:JSON.stringify({model,max_tokens:isRegistration?12000:7000,system:system+'\n'+continuationGuidance+(isRegistration?'\n'+registrationGuidance+'\n'+publicationGuidance:''),messages:[{role:'user',content}],tools:[{name:'submit_analysis',description:'研究の要約、提案、添削結果を構造化して返す',...(isRegistration?{strict:true}:{}),input_schema:isRegistration?registrationSchema:schema}],tool_choice:{type:'tool',name:'submit_analysis'}}),signal:AbortSignal.timeout(130000)});
       console.log('Claude request finished',{mode:body.mode,status:response.status,durationMs:Date.now()-aiStartedAt});
       if(!response.ok){const providerError=(await response.text()).slice(0,2000);console.error('Claude request rejected',{status:response.status,body:providerError});const message=response.status===429?'AIの利用上限に達しました。時間をおいて再試行してください。':response.status===401?'AI接続キーを確認してください。':response.status===402?'Claude APIの利用残高または支払い設定を確認してください。':response.status===400?'AIへの解析リクエスト形式に問題があります。LTI運営側で確認します。':'AI提供元に接続できませんでした。';if(registration?.fail)await registration.fail('anthropic_'+response.status);return reply({error:message},response.status===429?429:502);}
