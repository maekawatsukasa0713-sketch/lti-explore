...
export const registrationSchema={type:'object',properties:{title:{type:'string'},summary:{type:'array',minItems:1,items:{type:'string'}},suggestions:{type:'array',minItems:1,items:{type:'object',properties:{title:{type:'string'},description:{type:'string'},tags:{type:'array',items:{type:'string'}}},required:['title','description'],additionalProperties:false}},classification:{type:'object',properties:{field:{type:'string',enum:fields},reason:{type:'string'}},required:['field','reason'],additionalProperties:false}},required:['title','summary','suggestions','classification'],additionalProperties:false};
...
const registrationGuidance="一括登録の解析では、成績のような採点や5項目の評価は出力しません。使うのはタイトル・要約・継続研究案・分野分類です。corrections、advice、nextExperimentsは出力しません。要約は4〜6項目、各500字以内。継続研究案は原則3件、各350〜550字を目安にします。根拠のある提案を優先し、件数を合わせるために内容を水増ししません。文量より正確さを優先し、本文に根拠がない説明を補いません。";
...
export function normalizeRegistrationResult(v:any){
 ...
 return {issue:null,result:{title:v.title.trim(),summary:v.summary.map((s:string)=>s.trim()),suggestions,classification:{field,reason:v.classification.reason.trim()},corrections:[],advice:[],nextExperiments:[]}};
}
