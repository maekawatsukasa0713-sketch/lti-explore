...
const {evaluation:_evaluation,...registrationProperties}=schema.properties;
const registrationSchema={...schema,properties:{...registrationProperties,title:{type:'string',maxLength:200},summary:{type:'array',minItems:4,maxItems:6,items:{type:'string',maxLength:500}},suggestions:{type:'array',minItems:3,maxItems:3,items:{type:'object',properties:{title:{type:'string',maxLength:120},description:{type:'string',maxLength:1800},tags:{type:'array',items:{type:'string',maxLength:60},maxItems:3}},required:['title','description','tags'],additionalProperties:false}},corrections:{type:'array',items:{type:'string'},maxItems:0},advice:{type:'array',items:{type:'string'},maxItems:0},nextExperiments:{type:'array',items:{type:'string'},maxItems:0},classification:{type:'object',properties:{field:{type:'string',enum:fields},reason:{type:'string',maxLength:300}},required:['field','reason'],additionalProperties:false}},required:[...schema.required.filter((key:string)=>key!=='evaluation'),'classification']};
...
const registrationGuidance="一括登録の解析では、成績のような採点や5項目の評価は出力しません。使うのはタイトル・要約・継続研究案・分野分類です。corrections、advice、nextExperimentsは空の配列にしてください。要約は4〜6項目、各500字以内。継続研究案は3件ちょうどで、各350〜550字を目安にします。文量より正確さを優先し、本文に根拠がない説明を補いません。";
...
function registrationIssue(v:any){if(!v||typeof v!=='object')return 'missing_result';if(typeof v.title!=='string')return 'title';if(!Array.isArray(v.summary)||v.summary.length<4||v.summary.length>6||!v.summary.every((s:any)=>typeof s==='string'&&s.length<=500))return 'summary';if(!Array.isArray(v.suggestions)||v.suggestions.length!==3||!v.suggestions.every((s:any)=>s&&typeof s.title==='string'&&s.title.length<=120&&typeof s.description==='string'&&s.description.length<=1800&&Array.isArray(s.tags)&&s.tags.length<=3&&s.tags.every((t:any)=>typeof t==='string'&&t.length<=60)))return 'suggestions';if(!fields.includes(v.classification?.field))return 'classification_field';if(typeof v.classification?.reason!=='string'||v.classification.reason.length>300)return 'classification_reason';return null;}
  ...
  if(body.mode==='register'){
   const background=async()=>{
    ...
    try{
       ...
       const response=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'},body:JSON.stringify({model,max_tokens:isRegistration?12000:7000,system:system+'\n'+continuationGuidance+(isRegistration?'\n'+registrationGuidance+'\n'+publicationGuidance:''),messages:[{role:'user',content}],tools:[{name:'submit_analysis',description:'研究の要約、提案、添削結果を構造化して返す',input_schema:isRegistration?registrationSchema:schema}],tool_choice:{type:'tool',name:'submit_analysis'}}),signal:AbortSignal.timeout(130000)});
       ...
       const issue=isRegistration?registrationIssue(candidate):(!validResult(candidate)?'review_result':null);
       ...
       const result=isRegistration?{...candidate,corrections:[],advice:[],nextExperiments:[]}:candidate;
