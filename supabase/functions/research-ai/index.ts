...
const headers={'Access-Control-Allow-Origin':'https://lti-explore.vercel.app','Access-Control-Allow-Headers':'authorization, x-client-info, apikey, content-type, traceparent, tracestate, baggage','Access-Control-Allow-Methods':'POST, OPTIONS','Vary':'Origin','Content-Type':'application/json','Cache-Control':'no-store'};
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  // Counted in the database so the limit holds across function instances and restarts.
  ...
  const response=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'},body:JSON.stringify({model,max_tokens:4000,system,messages:[{role:'user',content}],tools:[{name:'submit_analysis',description:'研究の要約、提案、添削結果を構造化して返す',input_schema:body.mode==='register'?registrationSchema:schema}],tool_choice:{type:'tool',name:'submit_analysis'}}),signal:AbortSignal.timeout(55000)});
  if(!response.ok){const message=response.status===429?'AIの利用上限に達しました。時間をおいて再試行してください。':response.status===401?'AI接続キーを確認してください。':response.status===402?'Claude APIの利用残高または支払い設定を確認してください。':response.status===400?'AIが原稿を読み取れません。PDFのページ数・暗号化やモデル設定を確認してください。':'AI提供元に接続できませんでした。';if(registration?.fail)await registration.fail('anthropic_'+response.status);return reply({error:message},502);}
  const payload=await response.json();const result=payload.content?.find((c:any)=>c.type==='tool_use'&&c.name==='submit_analysis')?.input;
  if(payload.stop_reason==='max_tokens'||!validResult(result)||(body.mode==='register'&&(!fields.includes(result.classification?.field)||typeof result.classification?.reason!=='string'||result.classification.reason.length>500))){if(registration?.fail)await registration.fail('invalid_response');return reply({error:'AIの回答が不完全です。再試行してください。'},502);}
