...
const registrationEvaluationSchema={type:'object',properties:{scores:{type:'array',minItems:1,items:{type:'integer',enum:[0,1,2,3,4]}},reasons:{type:'array',minItems:1,items:{type:'string'}},evidence:{type:'array',minItems:1,items:{type:'string'}},nextSteps:{type:'array',minItems:1,items:{type:'string'}}},required:['scores','reasons','evidence','nextSteps'],additionalProperties:false};
...
const registrationGuidance="一括登録の解析では、タイトル・要約・継続研究案・分野分類に加え、LTI運営内部で確認する5観点評価も出力します。evaluationはscores・reasons・evidence・nextStepsの4配列で、各配列を必ず5件、順番を1問い・着眼点、2再現性、3手法の妥当性、4論理の一貫性、5発展性に固定します。scoresは0〜4で、0は本文情報不足による判断保留、1〜4は既存の4段階評価です。reasons・evidence・nextStepsは同じ順番で本文根拠に基づいて簡潔に書いてください。corrections、advice、nextExperimentsは出力しません。要約は4〜6項目、各500字以内。継続研究案は原則3件、各350〜550字を目安にします。根拠のある提案を優先し、件数を合わせるために内容を水増ししません。文量より正確さを優先し、本文に根拠がない説明を補いません。";
...
export function normalizeRegistrationResult(v:any){
 ...
 const scores=v.evaluation.scores,reasons=v.evaluation.reasons,evidence=v.evaluation.evidence,nextSteps=v.evaluation.nextSteps;
 if(![scores,reasons,evidence,nextSteps].every(x=>Array.isArray(x)&&x.length===5))return reject('evaluation_count');
 const evaluationItems=[];
 for(let i=0;i<5;i++){if(![0,1,2,3,4].includes(scores[i]))return reject(`evaluation_score_${i+1}`);if(![reasons[i],evidence[i],nextSteps[i]].every(x=>typeof x==='string'&&x.length<=2000))return reject(`evaluation_text_${i+1}`);evaluationItems.push({score:scores[i]===0?null:scores[i],reason:reasons[i].trim(),evidence:evidence[i].trim(),nextStep:nextSteps[i].trim()});}
