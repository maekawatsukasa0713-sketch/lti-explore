import {classificationPatch} from './research-fields.ts';
export async function claimPaper(base:string,headers:Record<string,string>,id:string,refreshEvaluation=false){
 ...
 const needsEvaluation=refreshEvaluation&&row.data.aiAnalysis&&!row.data.aiAnalysis.evaluation;
 if(row.data.aiAnalysis&&!needsEvaluation)return {result:row.data.aiAnalysis};
 if(row.data.aiState?.state==='processing')throw new Error('この成果は解析開始済みです。重複課金を防ぐため自動で再実行しません。');
 if(!needsEvaluation&&row.data.aiState&&row.data.aiState.state!=='failed')throw new Error('この成果は解析開始済みです。重複課金を防ぐため自動で再実行しません。');
