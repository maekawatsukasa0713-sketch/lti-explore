...
try{
 ...
 const registrationEvaluation={score1:3,reason1:'理由1',evidence1:'根拠1',nextStep1:'次1',score2:2,reason2:'理由2',evidence2:'根拠2',nextStep2:'次2',score3:3,reason3:'理由3',evidence3:'根拠3',nextStep3:'次3',score4:3,reason4:'理由4',evidence4:'根拠4',nextStep4:'次4',score5:4,reason5:'理由5',evidence5:'根拠5',nextStep5:'次5'};
 const normalized=normalizeRegistrationResult({title:'登録研究',summary:['要約'],suggestions:[{title:'次研究',description:'比較する',tags:[]}],classification:{field:'物理',reason:'物理現象'},evaluation:registrationEvaluation});assert.equal(normalized.issue,null);assert.equal(normalized.result.evaluation.items.length,5);assert.equal(normalized.result.evaluation.items[4].score,4);const held=normalizeRegistrationResult({title:'登録研究',summary:['要約'],suggestions:[{title:'次研究',description:'比較する',tags:[]}],classification:{field:'物理',reason:'物理現象'},evaluation:{...registrationEvaluation,score1:0}});assert.equal(held.result.evaluation.items[0].score,null);
