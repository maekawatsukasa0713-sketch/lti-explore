...
try{
 ...
 const {validResult,registrationSchema}=await import(path.join(dir,'ai.mjs'));
 assert.equal(registrationSchema.properties.summary.minItems,1);assert.equal(registrationSchema.properties.summary.maxItems,undefined);assert.equal(registrationSchema.properties.suggestions.minItems,1);assert.equal(registrationSchema.properties.suggestions.maxItems,undefined);\n const result={evaluation:{items:Array.from({length:5},(_,i)=>({score:3,reason:`参考理由${i}`,evidence:`本文の根拠${i}`,nextStep:`次の一歩${i}`}))},title:'試験用の研究',summary:['入力内容の要約'],suggestions:[{title:'追加の分析',description:'データの比較',tags:['分析']}],corrections:['方法を明記'],advice:['図の説明を追加'],nextExperiments:['既存データの比較']};
