...
export async function prepareResearch(id:number){
 let row=await readResearch(id);let paper=row.data;
 ...
 const accepted=await invokeResearchAI({mode:'register',paperId:String(id),title:paper.title,...input});
 if(!('accepted' in accepted))return accepted;
 const deadline=Date.now()+200000;
 while(Date.now()<deadline){
  await new Promise(resolve=>window.setTimeout(resolve,2000));
  const current=await readResearch(id);
  if(current.data.aiAnalysis)return current.data.aiAnalysis as ResearchAnalysis;
